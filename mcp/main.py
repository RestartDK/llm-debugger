...
def fetch_instructions_from_debugger() -> str:
    ...
    This tool reads the most recent instruction file from the instructions/ folder.
    It only considers files matching the timestamp pattern YYYY-MM-DD_HH-MM.txt.

    The retrieved instructions include:
    - Code chunks that need to be modified
    - File paths and line numbers
    - Specific changes to apply (before/after code)
    - Explanations of why changes are needed
    - Relationships between code chunks

    The instructions follow a structured format:
    [Code Chunk]
    File: <filepath>
    Lines: <start>-<end>
    ...
    [Relationships]
    <how this relates to other code chunks>

    Returns the complete instructions as a string from the most recent instruction file.
    Use these instructions to apply fixes to the codebase following the specified format.
    """
